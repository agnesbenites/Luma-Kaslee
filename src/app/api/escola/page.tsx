...
import { Loader2, Plus, Trash2, Users, ChevronDown, ChevronUp, AlertTriangle, X } from "lucide-react";
...
export default function TurmasPage() {
  ...
  async function carregar() {
    ...
    setProfessores(await profsRes.json());
    setCarregando(false);
  }
  ...
  function abrirModal() {
    setForm({ nome: "", serie: "", turno: "", professorId: "" });
    setErro("");
    setModalAberto(true);
  }
  ...
  async function handleCriar() {
    if (!form.nome || !form.serie) {
      setErro("Nome e série são obrigatórios.");
      return;
    }
    setSalvando(true);
    setErro("");
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-gray-500">Crie e gerencie as turmas da sua escola</p>
        </div>
        <button
          onClick={abrirModal}
      ...
      {sucesso && (
        <div className="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-2xl">✓ {sucesso}</div>
      )}
      ...
      {carregando ? (
        ...
        <div className="bg-white rounded-[2rem] border border-purple-100 p-16 text-center">
          ...
          <p className="font-bold text-gray-700">Nenhuma turma cadastrada</p>
          ...
          {turmas.map((turma) => (
            <div key={turma.id} className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden">
              <div className="flex items-center px-6 py-4">
                <button
                  onClick={() => setExpandido(expandido === turma.id ? null : turma.id)}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-bold">{turma.serie}</span>
                        {turma.turno && <span className="text-[10px] text-gray-400">{turma.turno}</span>}
                        {turma.professor && <span className="text-[10px] text-gray-400">· {turma.professor.nome}</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">{turma.alunos.length} aluno{turma.alunos.length !== 1 ? "s" : ""}</span>
                    {expandido === turma.id ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
                  </div>
                </button>
                <button
                  onClick={() => setConfirmarExclusao(turma.id)}
              ...
              {confirmarExclusao === turma.id && (
                <div className="mx-6 mb-4 p-4 bg-red-50 rounded-2xl border border-red-100 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-red-500 mt-0.5">Todos os dados desta turma serão removidos.</p>
                  </div>
                ...
                <div className="px-6 pb-6 border-t border-gray-50 pt-4 space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="bg-gray-50 rounded-2xl p-3">
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Série</p>
                      <p className="text-sm font-bold text-gray-700">{turma.serie}</p>
                    </div>
                    <div className="bg-gray-50 rounded-2xl p-3">
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Turno</p>
                      <p className="text-sm font-bold text-gray-700">{turma.turno || "—"}</p>
                    </div>
                    <div className="bg-gray-50 rounded-2xl p-3">
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Professor</p>
                      <p className="text-sm font-bold text-gray-700">{turma.professor?.nome || "—"}</p>
                    </div>
                    <div className="bg-gray-50 rounded-2xl p-3">
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Alunos</p>
                      <p className="text-sm font-bold text-gray-700">{turma.alunos.length}</p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {turma.alunos.map((aluno) => (
                          <span key={aluno.id} className="text-xs bg-white border border-gray-100 text-gray-600 px-3 py-1.5 rounded-xl">
                            {aluno.nome}
                          </span>
                        ))}
                      </div>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8 space-y-5">
            ...
            <div>
              ...
              <input type="text" value={form.nome} onChange={(e) => setForm((f) => ({ ...f, nome: e.target.value }))}
                placeholder="Ex: Turma A, 7B, Azul..."
            ...
            <div>
              ...
              {professores.length === 0 && (
                <p className="text-[10px] text-gray-400 mt-1">Nenhum professor cadastrado ainda. Convide professores primeiro.</p>
              )}
            </div>
            ...
            <button onClick={handleCriar} disabled={salvando}
              ...
              {salvando ? <><Loader2 size={16} className="animate-spin" /> Criando...</> : <><Plus size={16} /> Criar Turma</>}
            </button>
          </div>
        </div>
      )}
